...
let ``GetForAvatar.It returns a list of messages for an avatar that has messages.`` () =
    ...
    try
        ...
        let expected : Result<string list, string> =
            ...
            |> Ok
        let actual =
            MessageLegacy.GetForAvatar connection inputAvatarId
...
let ``ClearForAvatar.It clears out messages for an avatar that has messages.`` () =
    ...
    try
        ...
        let expected : Result<unit, string> =
            ...
            |> Ok
        let actual =
            MessageLegacy.ClearForAvatar connection inputAvatarId
        Assert.AreEqual(expected, actual)
        match MessageLegacy.GetForAvatar connection inputAvatarId with
        | Ok [] ->
            ()
        | _ ->
...
let ``AddToAvatar.It adds messages for a given avatar id.`` () =
    ...
    try
        ...
        let expected : Result<unit, string> =
            ...
            |> Ok
        let actual =
            MessageLegacy.AddForAvatar connection (inputAvatarId, inputMessage)
        Assert.AreEqual(expected, actual)
        match MessageLegacy.GetForAvatar connection inputAvatarId with
        | Ok [ _ ] ->
            ()
