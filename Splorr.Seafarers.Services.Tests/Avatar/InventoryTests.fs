...
let ``AddInventory.It adds a given number of given items to the given avatar's inventory.`` () =
    ...
    let context = TestAvatarAddInventoryContext(avatarInventorySink, avatarInventorySource) :> AvatarInventory.AddInventoryContext
    input
...
let ``RemoveInventory.It does nothing.When given a quantity of 0 items to remove or the given avatar has no items.`` 
    ...
    let context = TestAvatarRemoveInventoryContext(avatarInventorySink, avatarInventorySource) :> AvatarInventory.RemoveInventoryContext
    input
...
let ``GetItemCount.It returns zero when the given avatar has no entry for the given item.`` () =
    ...
    let avatarInventorySource (_) =
        Map.empty
    let context = TestAvatarGetItemCountContext(avatarInventorySource) :> AvatarInventory.GetItemCountContext
    let actual =
        input
        |> AvatarInventory.GetItemCount 
            context
            inputItem
...
let ``GetUsedTonnage.It calculates the used tonnage based on inventory and item descriptors.`` () =
    ...
    let avatarInventorySource (_) =
        ...
        |> Map.add 1UL 3UL
        |> Map.add 2UL 4UL
    let context = TestAvatarGetUsedTonnageContext(avatarInventorySource) :> AvatarInventory.GetUsedTonnageContext
    let actual =
        input
